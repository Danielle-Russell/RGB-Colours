...
class App extends React.Component {
  ...
  render() {
    ...
    var q1 = colors.slice(0, colors.length / 8);
    var q2 = colors.slice(colors.length / 8, colors.length / 4);
    var q3 = colors.slice(colors.length / 4, half);
    var q4 = colors.slice(half, colors.length);
    ...
    return (
      <div className="App">
        ...
        <div>
          {this.shuffleColors(q4).map((color) => (
            <p key={color} style={{ backgroundColor: color }}>
              .
            </p>
          ))}
